    ...
    QWidget(parent), actor(aktor), commandset(nullptr)
{
	...
	for (i = 0; i < commandsetLength; ++i) {
		if (commandset[i].length == LegacyStellantriebeDevice::Command_t::CommandLength::text) {
            ...
            if (commandset[i].writeAccess == LegacyStellantriebeDevice::Command_t::WriteAccess::read_only) {
...
void FeldbusAktorView::onUpdateDeviceValues(void) {
    for (CommandsetEntry& entry : commandsetGrid) {
        ...
        if (commandset[entry.key-1].writeAccess == LegacyStellantriebeDevice::Command_t::WriteAccess::write_only) {
            QPalette pal = entry.value->palette();
